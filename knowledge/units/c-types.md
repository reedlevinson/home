...
Examine [data-types.c](https://github.com/CS50DartmouthSP25/examples/blob/main/data-types.c), which defines variables of each of the base types and prints the `sizeof` each one.
...
For example, we see the following *declaration* in [readline.h](https://github.com/CS50DartmouthSP25/examples/blob/main/readline.h):

```c
extern bool readLine(char* buf, const int len);
```

and then in [readline.c](https://github.com/CS50DartmouthSP25/examples/blob/main/readline.c) we see the *definition* (implementation) of that function:
