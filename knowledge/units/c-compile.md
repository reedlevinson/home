...
***Warning:*** Be sure you don't mixup the arguments to `mygcc`(or`gcc`):

This is correct:

```bash
$ mygcc -o myprogram myprogram.c
```

However, this is a disaster! 

```bash
$ mygcc -o myprogram.c myprogram
```

The `gcc` program handles its arguments first.  When it encounters the
`-o` argument, it immediately checks to see if that file exists and,
if so, **erases it**. In the disaster scenario above, `gcc` will first
erase `myprogram.c` and then search for a file named `myprogram` to
attempt to compile it.  Yes, `gcc` does _exactly_ what you ask it to
do!
