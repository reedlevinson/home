...
For an example using `realloc`, read about [how readLinep() works](https://github.com/CS50DartmouthFA2025/home/blob/main/knowledge/units/readlinep.md).
...
Let's revise our [sorter1.c](https://github.com/CS50DartmouthFA2025/examples/blob/main/sorter1.c) program, using an array of pointers instead of a two-dimensional array of characters: the result is [sorter2.c](https://github.com/CS50DartmouthFA2025/examples/blob/main/sorter2.c).
...
You may want to read more about the [implementation](https://github.com/CS50DartmouthFA2025/home/blob/main/knowledge/units/readlinep.md) of `readLinep()`, which is a good example of character-by-character input, string construction, and use of `realloc` to grow an array.

## Example: sorter3

We can extend [sorter2.c](https://github.com/CS50DartmouthFA2025/examples/blob/main/sorter2.c) to sort the array using the library function `qsort()`, which implements the quicksort algorithm, resulting in [sorter3.c](https://github.com/CS50DartmouthFA2025/examples/blob/main/sorter3.c).
...
**Key point**: there should be exactly one call to `free()` for each pointer returned by `malloc()` and friends; and zero calls to `free()`  for pointers not produced by `malloc()` and friends.
...
> When the process exits, all its memory is freed - the four segments (code, global, stack, and heap) disappear.
> Nonetheless, in CS50 we expect you to clean up your mess before you exit - there should be no un-free'd memory remaining!

An upcoming unit presents the [valgrind](https://github.com/CS50DartmouthFA2025/home/blob/main/knowledge/units/valgrind.md) tool, which helps track memory usage and identify various kinds of memory errors.
