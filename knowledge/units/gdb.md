...
If you do use `printf` debugging, please use the C preprocessor to conditionally turn on/off the debugging output with one switch; that is, using `#ifdef DEBUG` and [conditional compilation](https://github.com/CS50DartmouthFA2025/home/blob/main/knowledge/units/c-conditional-compilation.md).
...
See our [gdb resources](https://github.com/CS50DartmouthFA2025/home/blob/main/logistics/systems.md#gdb) for manuals and tutorial documents.
...
If your program needs some command-line arguments, you can either pass them in on the `gdb` line:
...
or you can provide them on the `run` subcommand after you have entered `gdb`:
...
We will be working with [bugsort.c](https://github.com/CS50DartmouthFA2025/examples/blob/main/bugsort.c).
...
### Where does `gdb` find your source files?

When you start `gdb`, it assumes that the source code file for the program being debugged is in the current directory.
That's usually fine, but eventually (in labs 3-6 and the project) you will have source code files in other directories. 
In order to `gdb` find those source code files, you have to add them to `gdb`'s list of directories to search using the
`set directories` subcommand. First, find out where `gdb` is looking for source files with:
...
where `$cdir` refers to the directory where the file was compiled (if the compiler saved that information), and `$cwd` refers to the current working directory. Add another directory to that search list using:

```bash
...
(gdb) set directories ../tools/ 
(gdb) show directories
Source directories searched: /thayerfs/home/d29265d/_private/tmp/../tools:$cdir:$cwd
(gdb)
```

The directories are separated by `:`'s and searched in the order specified. 
You may reset the directories list to the default using
