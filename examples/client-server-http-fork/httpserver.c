// The code in this file is not quite up to current CS50 style guidelines.
/* 
 * httpserver - an example HTTP server.
 *
 * This program accepts connections forever in an infinite loop.
 * It forks a new process to handle each new client.
...
main(const int argc, char *argv[])
{
  ...
  // Accept connections, and fork to allow concurrent sessions
  while (true) {
  ...
  /* Note the main server process never explicitly exits, or closes list_sock,
   * but that socket will be closed when the program is killed or 
   * terminates. */
}
